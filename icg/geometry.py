    ...
    def __init__(self, mesh_dir=ROOT / "data/instrument_mesh", faces_per_part=0, load_appearance=False,
        ...
        for i, part in enumerate(PARTS):
            ...
            if load_appearance and self.part_albedo is None:
                from icg.appearance import load_material_mesh
...
class SemanticRenderer(nn.Module):
    ...
    def nvdiffrast_clip(self, vertices, k):
        """OpenCV camera vertices [B,N,3] to nvdiffrast clip coordinates.

        Same transform as ``_nvdiffrast``: pixel-centre projection, image-row
        flip encoded in the y clip component, OpenGL-style depth.
        """
        h, w = self.raster_size
        p = vertices @ k.transpose(-1, -2)
        z = vertices[..., 2]
        return torch.stack((2 * (p[..., 0] + .5 * z) / w - z,
                            z - 2 * (p[..., 1] + .5 * z) / h,
                            (self.far + self.near) / (self.far - self.near) * z -
                            2 * self.far * self.near / (self.far - self.near), z), -1).contiguous()

    @torch.no_grad()
    def render_icg(self, pose, k):
        """Silhouette, camera-space XYZ, and body IDs for ICG correspondence.

        Reuses the verified nvdiffrast clip transform. XYZ is interpolated from
        articulated mesh vertices so contour points sit on the true surface.
        """
        vertices, tips = self.mesh(pose)
        if k.ndim == 2:
            k = k[None].expand(len(vertices), -1, -1)
        ...
        raster_k[:, :2, 2] += (self.supersample - 1) / 2
        if self.backend != "nvdiffrast":
            raise ValueError("ICG correspondence rendering requires nvdiffrast")
        h, w = self.raster_size
        clip = self.nvdiffrast_clip(vertices, raster_k)
        ...
        for arm in range(self.mesh.arms):
            for part_i, count in enumerate(self.mesh.counts):
                ids.append(torch.full((count, 1), float(arm * 4 + part_i),
                                      device=vertices.device, dtype=vertices.dtype))
        part_ids = torch.cat(ids, 0)
        part_attr, _ = self.dr.interpolate(part_ids[None].contiguous(), rast, faces)
        ...
        return {
            ...
            "tips_camera": tips,
            "visible": (color[:, :c].sum(1, keepdim=True) > 0.5),
        }

    def _nvdiffrast(self, vertices, k,pose=None):
        h, w = self.raster_size
        clip = self.nvdiffrast_clip(vertices, k)
        faces = self.raster_faces
        rast, _ = self.dr.rasterize(self.ctx, clip, faces, resolution=[h, w],grad_db=False)
